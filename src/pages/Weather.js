          ...
          {activeTab === 'tenday' && daily && (
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.15)', overflow: 'hidden' }}>
              ...
              {daily.time.map((date, i) => {
                ...
                return (
                  <div key={i} style={{
                    display: 'flex', alignItems: 'center', padding: '15px 24px', gap: 12,
                    borderBottom: '1px solid rgba(255,255,255,0.07)',
                    background: i === 0 ? 'rgba(255,255,255,0.08)' : 'transparent',
                  }}>
                    {/* Day */}
                    <div style={{ minWidth: 100 }}>
                      <div style={{ fontSize: 14, fontWeight: i === 0 ? 600 : 400 }}>{getDayName(date, i)}</div>
                      <div style={{ fontSize: 12, opacity: 0.65, direction: 'rtl' }}>{getDayNameUrdu(date, i)}</div>
                      <div style={{ fontSize: 11, opacity: 0.45 }}>{date}</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 160 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 12, opacity: 0.8 }}>{w.label}</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 80 }}>
                      <span style={{ fontSize: 12, color: '#4fc3f7', minWidth: 34 }}>{daily.precipitation_probability_max[i]}%</span>
                      <div style={{ width: 50, height: 4, background: 'rgba(255,255,255,0.15)', borderRadius: 2 }}>
                        <div style={{ width: `${daily.precipitation_probability_max[i]}%`, height: '100%', background: '#4fc3f7', borderRadius: 2 }} />
                      </div>
                    </div>

                    {/* Temp bar */}
                    <div style={{ display: 'flex', alignItems: 'center', flex: 1, minWidth: 180 }}>
                      <span style={{ fontSize: 14, minWidth: 32, opacity: 0.75 }}>{low}°</span>
                      <TempBar low={low} high={high} globalLow={globalLow} globalHigh={globalHigh} />
                      <span style={{ fontSize: 14, minWidth: 32, textAlign: 'right', fontWeight: 600 }}>{high}°</span>
                    </div>

                    {/* Wind */}
                    <div style={{ fontSize: 12, opacity: 0.6, minWidth: 80, textAlign: 'right' }}>
